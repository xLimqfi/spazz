...
module.exports = {
  ...
  if (!args[0]) return message.channel.send(`Please specify a GitHub username!`)
    
  fetch(`https://api.github.com/users/${args.join('-')}`)
    .then(res => res.json()).then(body => {
      if(body.message) return message.channel.send(`User Not Found | Please specify a GitHub username!`);
    let { login, avatar_url, name, id, html_url, public_repos, followers, following, location, created_at, bio } = body;

            const embed = new MessageEmbed()
            .setAuthor(`${login} Github Information!`, avatar_url)
            .setColor(`#211F1F`)
